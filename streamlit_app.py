...
# Setup placeholders for the progress bar and loading text
title = st.empty()
...
# Simulate long computation for the loading bar
for i in range(100):
...
# Display the final loading message
...
with col2:
    ...
    grading_system = st.select_slider('grading system', options=[4, 5, 7, 10], format_func=lambda x: f"{x}")
...
# Prediction button and processing logic
if st.button('Show CGPA'):
    ...
    st.write(predictions * gpa_data_comp_col["grading_system"].values[0])  # Scale prediction by grading system
